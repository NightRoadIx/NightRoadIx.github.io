...
Neuron.prototype.connectNeuronTo = function ( neuronB ) {
	...
	var neuronA = this;
	// create axon and establish connection between neurons
...
// Particle Pool, to simulate synapses
...
function NeuralNetwork() {
	...
	// the axon
	this.axonOpacityMultiplier = 0.8;
	...
	// the neurons
	this.neuronSizeMultiplier = 1.5;
	this.spriteTextureNeuron = TEXTURES.electric;
	this.neuronColor = '#00ff33';
...
NeuralNetwork.prototype.initNeurons = function ( inputVertices ) {
	...
	for ( i = 0; i < this.components.neurons.length; i++ ) {
		this.neuronAttributes.color.value[ i ] = new THREE.Color( '#00ff33' ); // initial neuron color
...
function initGui() {
	...
	gui_info = gui.addFolder( 'Informacion' );
	gui_info.add( neuralNet, 'numNeurons' ).name( 'Neuronas' );
	gui_info.add( neuralNet, 'numAxons' ).name( 'Axones' );
	gui_info.add( neuralNet, 'numSignals', 0, neuralNet.settings.limitSignals ).name( 'Señales' );
	gui_info.autoListen = false;

	gui_settings = gui.addFolder( 'Configuraciones' );
	gui_settings.add( neuralNet.settings, 'currentMaxSignals', 0, neuralNet.settings.limitSignals ).name( 'Max Señales' );
	gui_settings.add( neuralNet.particlePool, 'pSize', 0.2, 2 ).name( 'Tamaño Señal' );
	gui_settings.add( neuralNet.settings, 'signalMinSpeed', 0.0, 8.0, 0.01 ).name( 'Señal Min Velocidad' );
	gui_settings.add( neuralNet.settings, 'signalMaxSpeed', 0.0, 8.0, 0.01 ).name( 'Señal Max Velocidad' );
	gui_settings.add( neuralNet, 'neuronSizeMultiplier', 0, 2 ).name( 'Tamaño Neurona Mult' );
	gui_settings.add( neuralNet, 'neuronOpacity', 0, 1.0 ).name( 'Neuron Opacidad' );
	gui_settings.add( neuralNet, 'axonOpacityMultiplier', 0.0, 5.0 ).name( 'Opacidad Axon Mult' );
	gui_settings.addColor( neuralNet.particlePool, 'pColor' ).name( 'Color Señal' );
	gui_settings.addColor( neuralNet, 'neuronColor' ).name( 'Color Neurona' );
	gui_settings.addColor( neuralNet, 'axonColor' ).name( 'Color Axon ' );
	gui_settings.addColor( sceneSettings, 'bgColor' ).name( 'Fondo' );
